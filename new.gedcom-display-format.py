...
def get_individuals( who_to_include, person_id, id_item ):
    ...
    if who_to_include == 'all':
       ...
       if the_person is not None:
          ...
          print( 'Selected person', the_person, '=', get_name(data[ikey][the_person], 'display'), file=sys.stderr )
...
def output_json():
    ...
    if options['include'] == 'ancestors':
       output[the_person] = json_ancestors( the_person )

    else:
       output[the_person] = json_descendents( the_person )
...
def options_ok( program_options ):
    ...
    if program_options['format'] == 'json':
       ...
       if program_options['include'] in exclude:
          print( 'JSON format is not compatible with including', exclude, file=sys.stderr )
          result = False
